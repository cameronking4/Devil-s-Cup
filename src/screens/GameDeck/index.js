...
class GameDeck extends Component {
    ...
    getRandomColor = () => {
      const colors = ['rgba(20, 100, 211, 1)', 'rgb(82, 147, 237)', 'rgb(148, 185, 237)']
    ...
    render () {
      ...
      let color = this.state.color.interpolate({
        inputRange: [0, 255],
        outputRange: ['rgba(20, 100, 211, 1)', 'rgb(148, 185, 237)']
      })

      let btnColor = this.state.btnColor.interpolate({
        inputRange: [0, 255],
        outputRange: ['rgba(20, 100, 211, 1)', 'rgb(82, 147, 237)']
      })
      ...
      return (
        <Animated.View style={styles.container}>
          ...
          <Button
            mode='outlined'
            color='#fff'
            onPress={btnHandler} style={[styles.btn, {backgroundColor: '#000' }]} type='contained' dark>{btnText}</Button>
        </Animated.View>
      )
    }
}
